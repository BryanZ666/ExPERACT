...
import utils
from utils import visualise_voxel, stack_on_channel
from voxel.voxel_grid import VoxelGrid
from voxel.augmentation import apply_se3_augmentation
from einops import rearrange
from clip.core.clip import build_model, load_clip
...
class QFunction(nn.Module):
    ...
    def forward(self, rgb_pcd, proprio, pcd, lang_goal_emb, lang_token_embs,option,
...
class QAttentionPerActBCAgent(Agent):
    ...
    def update(self, steps,obs, pcd, proprio, action_trans,action_rot_grip,action_gripper_pose, \
        ...
        if steps%3000==0:
          ...
          z_argmax = np.argmax(q_rot_z_flat.detach().cpu().numpy(),axis=1)
          grip_argmax = np.argmax(q_grip_flat.detach().cpu().numpy(),axis=1)
          rot_grip_argmax = [[int(x_argmax), int(y_argmax), int(z_argmax),int(grip_argmax)]]

          q_trans_argmax = np.argmax(q_trans_flat.detach().cpu().numpy(),axis=1)
          gt_trans_flat = np.argmax(action_trans_one_hot_flat.detach().cpu().numpy(),axis=1)
          compare_trans = [[int(q_trans_argmax), int(gt_trans_flat)]]
    ...
    def act(self, obs, pcd, proprio, lang_goal_emb,lang_token_embs,\
        ...
        # print("Eval--pcs shape: ",pcd[0].shape)
