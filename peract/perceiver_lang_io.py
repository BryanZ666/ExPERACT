...
from basemodel import DenseBlock, SpatialSoftmax3D, Conv3DBlock, Conv3DUpsampleBlock
    ...
    def forward(
        ...
        if self.lang_fusion_type == 'seq':
            ...
            if option is not None:
              option = option.unsqueeze(1).repeat(1, l.shape[1], 1)
              # l = torch.cat([l,option],dim=1)              # [B,154,128]
              l = option             # [B,77,128]
            ins = torch.cat((l, ins), dim=1)                  # [B,8077,128] or [B,8154,128]
